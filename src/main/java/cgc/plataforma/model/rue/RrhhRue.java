 ...
 */
package cgc.plataforma.model.rue;
...
@NamedNativeQueries({
    @NamedNativeQuery(name = "RrhhRue.NativeQueryUsuario",
                      query = "SELECT DISTINCT R.ID_RUE rue,'ACTIVO DESDE ' || TO_CHAR(L.FECHA_INICIO, 'DD MONTH YYYY') fechaActivo, " +
                            "       R.PRIMER_NOMBRE || ' ' || R.PRIMER_APELLIDO AS nombre, " +
                            "       P.NOMBRE_FUNCIONAL puestoFuncional " +
                            "FROM RRHH_RUE R " +
                            "INNER JOIN RRHH_LABORAL L ON R.ID_RUE = L.ID_RUE " +
                            "INNER JOIN RRHH_PUESTO_FUNCIONAL P ON L.PUESTO_FUNCIONAL = P.PUESTO_FUNCIONAL " +
                            "INNER JOIN ZEUS_PERFIL_USUARIO PU ON R.ID_RUE = PU.ID_RUE "+  
                            "WHERE UPPER(PU.LOGIN_AD) = UPPER(?) " +
                            "AND L.ESTADO =  'A' ",
                      resultSetMapping = "ResultsUser"
    ),
    @NamedNativeQuery(name = "RrhhRue.NativeQueryImg",
                      query = "SELECT IMAGEN idImagen, "+
                              "ARCHIVO archivoImg "+
                              "FROM RRHH_IMAGEN "+
                              "WHERE ID_RUE = ? "+
                              "ORDER BY FECHA_INSERT DESC",
                      resultSetMapping = "ResultsImg"
    ),
     @NamedNativeQuery(name = "RrhhRue.NativeQueryRueLaboral",
                      query = "SELECT DISTINCT R.ID_RUE, " +                            
                            "       L.RENGLON " +
                            "FROM RRHH_RUE R " +
                            "INNER JOIN RRHH_LABORAL L ON R.ID_RUE = L.ID_RUE " +
                            "INNER JOIN RRHH_PUESTO_FUNCIONAL P ON L.PUESTO_FUNCIONAL = P.PUESTO_FUNCIONAL " +
                            ...
                            "WHERE UPPER(PU.LOGIN_AD) = UPPER(?) " +
                            "AND L.ESTADO =  'A' ",
                      resultSetMapping = "ResultsLaboral"
    )
})
@SqlResultSetMappings({
    @SqlResultSetMapping(name="ResultsUser",
                         classes = {@ConstructorResult(targetClass = ResultsUser.class,
                                 columns = {@ColumnResult(name = "rue"),
                                            @ColumnResult(name = "fechaActivo"),
                                            @ColumnResult(name = "nombre"),
                                            @ColumnResult(name = "puestoFuncional") })
                         }),
     @SqlResultSetMapping(name="ResultsImg",
                         classes = {@ConstructorResult(targetClass = ResultsImg.class,
                                 columns = {@ColumnResult(name = "idImagen"),
                                            @ColumnResult(name = "archivoImg") })
                         }),
    @SqlResultSetMapping(name="ResultsLaboral",
                         classes = {@ConstructorResult(targetClass = ResultsLaboralRue.class,
                                 columns = {@ColumnResult(name = "ID_RUE", type = Integer.class),
                                            @ColumnResult(name = "RENGLON", type = Integer.class) })
                         })
})
public class RrhhRue implements Serializable {
    ...
    public String toString() {
        return "cgc.plataforma.model.rue.RrhhRue[ idRue=" + idRue + " ]";
    }
